...
public class ParseCurlCommandActionTest {
    ...
    public void testCreateSampler() throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, IOException {
        ...
        request = basicCurlParser
                .parse("curl 'http://jmeter.apache.org/' --data 'name=test'");
        ...
        request = basicCurlParser
                .parse("curl 'http://jmeter.apache.org/' -F 'c=@"+filePath+";type=text/foo' -F 'c1=@"+filePath+"'");
        objs = new Object[] { request, "" };
        httpSampler = (HTTPSamplerProxy) method.invoke(p, objs);
        assertEquals("form name should be set in httpsampler", "c",
                httpSampler.getHTTPFiles()[0].getParamName());
        assertEquals("form name should be set in httpsampler", filePath,
                httpSampler.getHTTPFiles()[0].getPath());
        assertEquals("form name should be set in httpsampler", "c1",
                httpSampler.getHTTPFiles()[1].getParamName());
        
        //test form data in httpsampler
        request = basicCurlParser
                .parse("curl 'http://jmeter.apache.org/' --form-string 'c=@test.txt;type=text/foo'");
        objs = new Object[] { request, "" };
        httpSampler = (HTTPSamplerProxy) method.invoke(p, objs);
        assertEquals("form name should be set in httpsampler", "c",
                httpSampler.getArguments().getArgument(0).getName());
    ...
    public void testCreateHeaderManager()
        ...
        assertEquals("header should be set in HeaderManager", "Content-Type",
                headerManager.get(0).getName());
        assertEquals("header should be set in HeaderManager", "application/x-www-form-urlencoded",
                headerManager.get(0).getValue());
        assertEquals("header should be set in HeaderManager", "Accept-Encoding",
                headerManager.get(1).getName());
        assertEquals("header should be set in HeaderManager", "gzip, deflate",
                headerManager.get(1).getValue());
    }
    ...
    public void testCreateAuthManager()
        ...
        BasicCurlParser basicCurlParser = new BasicCurlParser();
        Request request = basicCurlParser.parse(
                "curl 'http://jmeter.apache.org/' -u 'user:passwd'");
        Class<ParseCurlCommandAction> parseCurlCommandAction = ParseCurlCommandAction.class;
        ...
        HeaderManager headerManager = (HeaderManager) method.invoke(p, objs);
        assertEquals("the username of Authorization should be set in AuthManager", "user", authManager.get(0).getUser());
        assertEquals("the password of Authorization should be set in AuthManager", "passwd", authManager.get(0).getPass());
    }
    ...
    public void testCreateCookieManager()
        ...
        method.invoke(p, objs);
        assertEquals("the domain of cookie should be set in cookieManager", "jmeter.apache.org", cookieManager.get(0).getDomain());
    ...
    public void testCreateCookieManagerHeader()
            throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, NoSuchFieldException, SecurityException {
        ParseCurlCommandAction p = new ParseCurlCommandAction();
        CookieManager cookieManager=new CookieManager();
        BasicCurlParser basicCurlParser = new BasicCurlParser();
        ...
        objs = new Object[] { cookieManager,request};
        method.invoke(p, objs);
        assertTrue("When doesn't choose to upload cookies from header,"
                + "the cookie shouldn't be set in cookieManager", cookieManager.getCookies().size()==0);
    }
    @Test
    public void testDnsServer()
        ...
        method.invoke(p, objs);
        assertEquals("the dns server should be set in DNSCacheManager", "0.0.0.0",  dnsCacheManager.getServers().get(0).getStringValue());
    }
